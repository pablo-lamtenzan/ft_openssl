...
/*   Updated: 2020/10/20 23:20:52 by pablo            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void				sll_sha256_crypt_message(unsigned int* vectors, unsigned int* chunks)
{
	...
	{
		tmps[0] = vec_rot_right(vectors[A], 2) ^ vec_rot_right(vectors[A], 13) \
				^ vec_rot_right(vectors[A], 22);
		tmps[1] = vec_rot_right(vectors[E], 6) ^ vec_rot_right(vectors[E], 11) \
			^ vec_rot_right(vectors[E], 25);
...
static void				cp_array_u32bits(unsigned int* dest, unsigned int* src)
{
	...
	while (++iterator < 64)
	{
		x = vec_rot_right(dest[iterator - 15], 7) \
			^ vec_rot_left(dest[iterator - 15], 18) ^ (dest[iterator -15] >> 3);
		y = vec_rot_right(dest[iterator - 2], 17) \
			^ vec_rot_left(dest[iterator - 2], 19) ^ (dest[iterator -15] >> 10);
...
{
	const int			chunks_nb = sha256->bits / CHUNK_BIT_SIZE;
	unsigned int*		tmp;
	int					chunk_index;
	int					update_index;
...
const char*				sll_sha256(const char *data)
{
	...
	if (!(message = ft_calloc(sizeof(char), bytes)) \
		|| !(digest = ft_calloc(sizeof(sha256.buff))))
		return ((void*)0);
